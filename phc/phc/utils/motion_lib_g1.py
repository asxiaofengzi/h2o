...
from phc.utils.torch_g1_humanoid_batch import Humanoid_Batch  # 使用G1的Humanoid_Batch
...
class MotionLibG1(MotionLibBase):
    ...
    def __init__(self, motion_file, device, fix_height=FixHeightMode.no_fix, masterfoot_conifg=None, min_length=-1, im_eval=False, multi_thread=True, extend_hand=True, extend_head=False, mjcf_file="legged_gym/resources/robots/g1/g1_23dof.xml", sim_timestep=1/50):
        """
        初始化G1机器人运动库
        ...
        参数:
            ...
            device: 计算设备（CPU或GPU）
            fix_height: 高度修复模式
            masterfoot_conifg: 足部控制配置
            min_length: 最小运动长度
            im_eval: 是否为评估模式
            multi_thread: 是否使用多线程
            extend_hand: 是否扩展手部
            extend_head: 是否扩展头部
            mjcf_file: G1机器人的MJCF文件路径
            sim_timestep: 模拟时间步长
        """
    ...
    def fix_trans_height(pose_aa, trans, curr_gender_betas, mesh_parsers, fix_height_mode):
        ...
        参数:
            pose_aa: 姿势角度
            trans: 位置
            curr_gender_betas: 性别和形状参数
            mesh_parsers: 网格解析器
            fix_height_mode: 高度修复模式
        ...
        with torch.no_grad():
            raise NotImplementedError("G1机器人不支持高度修复")
            return trans, 0
    ...
    def load_motions(self, skeleton_trees, gender_betas, limb_weights, random_sample=True, start_idx=0, max_len=-1, target_heading=None):
        ...
        参数:
            skeleton_trees: 骨骼树列表
            gender_betas: 性别和形状参数列表
            limb_weights: 肢体权重列表
        ...
        print(f"已加载 {num_motions:d} 个运动，总时长 {total_len:.3f}秒，共 {self.gts.shape[0]} 帧。")
        return motions
    ...
    def get_motion_state(self, motion_ids, motion_times, offset=None):
        ...
        if "dof_pos" in self.__dict__:  # G1关节
    ...
    def load_motion_with_skeleton(ids, motion_data_list, skeleton_trees, gender_betas, fix_height, mesh_parsers, masterfoot_config, target_heading, max_len, queue, pid):
        ...
        参数:
            ids: 数据ID列表
            motion_data_list: 运动数据列表
            skeleton_trees: 骨骼树列表
            gender_betas: 性别和形状参数列表
            fix_height: 高度修复模式
            mesh_parsers: 网格解析器
            masterfoot_config: 足部控制配置
            target_heading: 目标朝向
            max_len: 最大长度
            queue: 多进程队列
            pid: 进程ID
